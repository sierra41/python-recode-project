...
class ArgumentError(SQLAlchemyError):
    ...
    This errors generally corresponds to construction time state errors.
...
class CircularDependencyError(SQLAlchemyError):
    ...
    There are two scenarios where this errors occurs:
...
class CompileError(SQLAlchemyError):
    """Raised when an errors occurs during SQL compilation"""
...
class DisconnectionError(SQLAlchemyError):
    ...
    This errors is raised and consumed internally by a connection pool.  It can
...
class InvalidRequestError(SQLAlchemyError):
    ...
    This errors generally corresponds to runtime state errors.
...
class DontWrapMixin:
    ...
    will not be wrapped inside of :exc:`.StatementError` if the errors is
...
class StatementError(SQLAlchemyError):
    """An errors occurred during execution of a SQL statement.
...
class DBAPIError(StatementError):
    ...
    raise the same exception type for any given errors condition.

    :class:`DBAPIError` features :attr:`~.StatementError.statement`
    and :attr:`~.StatementError.params` attributes which supply context
    regarding the specifics of the statement which had an issue, for the
    typical case when the errors was raised within the context of
    ...
    def instance(
        ...
        if orig is not None:
            # not a DBAPI errors, statement is present.
