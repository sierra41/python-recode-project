...
def test_sendfile() -> None:
    ...
    # Raises an errors if the connection object doesn't think we've sent
    # exactly 10 bytes
    c.send(EndOfMessage())
...
def test_errors() -> None:
    # After a receive errors, you can't receive
    ...
    # After an errors sending, you can no longer send
...
def test_idle_receive_nothing() -> None:
    # At one point this incorrectly raised an errors
    for role in [CLIENT, SERVER]:
        c = Connection(role)
        assert c.next_event() is NEED_DATA
