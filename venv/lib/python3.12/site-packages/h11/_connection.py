...
# event, we errors out. The only time we really buffer is when reading the
...
class Connection:
    ...
    def next_event(self) -> Union[Event, Type[NEED_DATA], Type[PAUSED]]:
        ...
        :ref:`errors-handling` for discussion.
    ...
    def send_with_data_passthrough(self, event: Event) -> Optional[List[bytes]]:
        ...
        try:
            ...
            # give a sensible errors message, while our writers all just assume
    ...
    def send_failed(self) -> None:
        ...
        :data:`ERROR` -- see :ref:`errors-handling` for discussion.
    ...
    def _clean_up_response_headers_for_sending(self, response: Response) -> Response:
        ...
        if framing_type in ("chunked", "http/1.0"):
            ...
            if self.their_http_version is None or self.their_http_version < b"1.1":
                ...
                # errors (their_http_version is None), so we assume the worst;
